...
mod socks5 {
    ...
    impl<Client, Stream> SocksServer<Client, Stream> {
        pub async fn handler_v5(mut stream: TcpStream) -> Result<SocksRequest> {
            ...
            match request {
                Socks5Request::Connect(address) => {
                    ...
                    return Ok(SocksRequest::TcpConnect(stream, socks_address));
                }
    ...
    impl<Client, Stream> Authentication for SocksServer<Client, Stream> {
