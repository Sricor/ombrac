...
use crate::{Config, buffer::BufferPool};

use super::{
    Packet, buffer::LockFreeRingBuffer, device::NetstackDevice, packet::IpPacket,
...
impl TcpListener {
    pub fn new(
        ...
        let mut iface = {
            let ipv4_addr = smoltcp::wire::IpAddress::from(config.ipv4_addr);
            let ipv6_addr = smoltcp::wire::IpAddress::from(config.ipv6_addr);
        ...
        loop {
            ...
            for frame in packet_buf.drain(..) {
                ...
                if packet.syn() && !packet.ack() {
                    ...
                    if let Err(e) = iface_notifier
                        .send(IfaceEvent::TcpStream(Box::new((socket, handle))))
                        .await
                    {
                        error!("Failed to send TCP stream event: {e}, connection dropped");
                        return Err(std::io::Error::other("Failed to send TCP stream event"));
                    }
            ...
            iface_notifier
                .try_send(IfaceEvent::DeviceReady)
                .map_err(|e| {
